    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <p className="mt-2 text-sm text-[#6A7893]">先完成今天最重要的复习，再整理新错题。</p>
        </div>
      ...
      {dashboard.isPending ? <div className="rounded-xl border border-[#E4E8F2] bg-white px-6 py-16 text-center text-sm text-[#6A7893]" role="status">正在加载今日学习概览…</div> : null}
        ...
        <>
          ...
          <div className="grid gap-5 lg:grid-cols-[minmax(0,0.8fr)_minmax(0,1.2fr)]">
            <Link href="/questions/new" className="flex min-h-32 items-center gap-4 rounded-xl border border-[#CDECE2] bg-white p-5 transition-colors hover:bg-[#F7FCFA] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#10B981]">
              <span className="flex size-11 items-center justify-center rounded-lg bg-[#E8F8F3] text-[#079976]" aria-hidden="true"><Plus className="size-6" /></span>
              <span><span className="block font-semibold text-[#079976]">快速录入</span><span className="mt-1 block text-sm leading-6 text-[#6A7893]">上传截图或手动记录新错题</span></span>
            </Link>
            <AiAdvice advice={dashboard.data.ai_advice} providerMode={dashboard.data.provider_mode} refreshing={dashboard.isFetching} onRefresh={() => dashboard.refetch()} />
          </div>
