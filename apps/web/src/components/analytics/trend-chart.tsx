  ...
  return (
    <section className="rounded-xl border border-[#E4E8F2] bg-white p-5" aria-labelledby="trend-chart-title">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div><h2 id="trend-chart-title" className="font-semibold text-[#0D1B4C]">错题录入趋势</h2><p className="mt-1 text-sm text-[#52627F]">当前周期共录入 {total} 题</p></div>
        <div className="flex rounded-lg border border-[#E4E8F2] bg-[#F7F8FC] p-1" role="group" aria-label="趋势周期">
          {([7, 30] as const).map((value) => <button key={value} type="button" aria-pressed={days === value} onClick={() => setDays(value)} className={`h-8 rounded-md px-3 text-xs font-medium focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#4F46E5] ${days === value ? "bg-white text-[#4F46E5] shadow-sm" : "text-[#52627F]"}`}>最近 {value} 天</button>)}
        </div>
      </div>
      <div className="mt-5 h-64 min-w-0" aria-hidden="true">
        <ResponsiveContainer width="100%" height="100%" minWidth={260} minHeight={256}>
          <LineChart data={data} margin={{ top: 8, right: 8, left: -20, bottom: 0 }}>
            ...
            <XAxis dataKey="date" tickFormatter={(value: string) => value.slice(5)} tick={{ fill: "#52627F", fontSize: 11 }} axisLine={false} tickLine={false} minTickGap={20} />
            <YAxis allowDecimals={false} tick={{ fill: "#52627F", fontSize: 11 }} axisLine={false} tickLine={false} />
      ...
      <div className="mt-4 max-h-44 overflow-auto rounded-lg border border-[#E8ECF4]">
        <table className="w-full text-left text-xs" aria-label={`最近 ${days} 天录入明细`}>
          <thead className="sticky top-0 bg-[#FAFBFD] text-[#52627F]"><tr><th className="px-3 py-2" scope="col">日期</th><th className="px-3 py-2 text-right" scope="col">录入题数</th></tr></thead>
